...
export function createChecklistTelegramHandler(kind: ChecklistKind) {
  ...
  return async function POST(request: NextRequest): Promise<NextResponse> {
    ...
    if (!material) {
      return NextResponse.json({ ok: false, message: `${config.nameLabel} ontbreekt.` }, { status: 400 });
    }
    if (material.length > MAX_MATERIAL_LENGTH) {
      return NextResponse.json({ ok: false, message: `${config.nameLabel} is te lang.` }, { status: 400 });
    }

    try {
      const { firestore } = initFirebaseAdmin();
      const listSnapshot = await firestore
        .collection(config.lists)
        .where('is_general', '==', true)
        .limit(1)
        .get();

      if (listSnapshot.empty) {
        return NextResponse.json({ ok: false, message: `${config.title} bestaat nog niet.` }, { status: 404 });
      }
      ...
      const itemRef = firestore.collection(config.items).doc(`telegram-${chatPart}-${messagePart}`);
      ...
      console.error(`Telegram ${config.title} opslaan mislukt:`, error);
      return NextResponse.json({ ok: false, message: `${config.itemLabel} kon niet worden opgeslagen.` }, { status: 500 });
    }
  };
}
