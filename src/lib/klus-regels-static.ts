...
function createVlieringRuleSet(): Record<string, Record<string, any>> {
  return {
    randbalken: {
      ...
      formula: 'aantal = ceil(((2 * floor_length_mm)) / material.lengte_mm)',
    ...
    vloerbalken: {
      ...
      formula: 'beam_count = ceil(floor_length_mm / balkafstand_mm) + 1; vloerbalk_total_mm = beam_count * floor_width_mm; raveel_extra_mm = sum((2 * opening_width_mm) + (2 * opening_height_mm) for opening in vlizotrap_openings); totale_lengte_mm = vloerbalk_total_mm + raveel_extra_mm; aantal = ceil((totale_lengte_mm) / material.lengte_mm)',
    ...
    balkdragers: {
      ...
      formula: 'beam_count = ceil(floor_length_mm / balkafstand_mm) + 1; interne_balken = max(0, beam_count - 2); basis = interne_balken * 2; raveel = vlizotrap_count * 4; aantal = ceil((basis + raveel))',
    ...
    beplating: {
      ...
      formula: 'vloer_bruto_m2 = (floor_length_mm * floor_width_mm) / 1000000; total_openings_area_m2 = sum(((opening_width_mm * opening_height_mm) / 1000000) for opening in openings_source); vloer_netto_m2 = max(0, vloer_bruto_m2 - total_openings_area_m2); plaat_m2 = material.lengte_m * material.breedte_m; aantal = ceil((vloer_netto_m2) / plaat_m2)',
    ...
    luik_afwerking: {
      ...
      formula: "vlizotrap_perimeter_m1 = sum((2 * (opening_width_mm + opening_height_mm)) / 1000 for opening in openings where opening.type in ['vlizotrap', 'hatch']); aantal = ceil((vlizotrap_perimeter_m1) / material.lengte_m)",
    ...
    koof_regelwerk: {
      ...
      formula: 'koof_total_lineair_mm = sum(((2 * koof.lengte) + (2 * koof.hoogte) + (2 * koof.diepte)) * (koof.aantal ?? 1) for koof in koof_items); aantal = ceil((koof_total_lineair_mm) / material.lengte_mm)',
    ...
    koof_constructieplaat: {
      ...
      formula: 'koof_total_surface_m2 = sum((((koof.lengte * koof.hoogte) + (koof.lengte * koof.diepte)) / 1000000) * (koof.aantal ?? 1) for koof in koof_items); plaat_m2 = material.lengte_m * material.breedte_m; aantal = ceil((koof_total_surface_m2) / plaat_m2)',
    ...
    koof_isolatie: {
      ...
      formula: 'koof_total_surface_m2 = sum((((koof.lengte * koof.hoogte) + (koof.lengte * koof.diepte)) / 1000000) * (koof.aantal ?? 1) for koof in koof_items); element_m2 = material.lengte_m * material.breedte_m; stuks = ceil((koof_total_surface_m2) / element_m2)',
      pack_handling: 'if materiaalnaam matches /(pak\\s*(\\d+)st|\\((\\d+)st)/i then aantal = ceil(stuks / pack_size) else aantal = stuks',
      required_inputs: ['maatwerk_item.koven', 'material.lengte', 'material.breedte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    ...
    isolatie_basis: {
      ...
      formula: 'vloer_bruto_m2 = (floor_length_mm * floor_width_mm) / 1000000; total_openings_area_m2 = sum(((opening_width_mm * opening_height_mm) / 1000000) for opening in openings_source); vloer_netto_m2 = max(0, vloer_bruto_m2 - total_openings_area_m2); element_m2 = material.lengte_m * material.breedte_m; stuks = ceil((vloer_netto_m2) / element_m2)',
      pack_handling: 'if materiaalnaam matches /(pak\\s*(\\d+)st|\\((\\d+)st)/i then aantal = ceil(stuks / pack_size) else aantal = stuks',
      required_inputs: ['maatwerk_item.lengte', 'maatwerk_item.breedte', 'material.lengte', 'material.breedte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    ...
    gips_vuller: {
      ...
      formula: 'vloer_bruto_m2 = (floor_length_mm * floor_width_mm) / 1000000; total_openings_area_m2 = sum(((opening_width_mm * opening_height_mm) / 1000000) for opening in openings_source); vloer_netto_m2 = max(0, vloer_bruto_m2 - total_openings_area_m2); if material.verbruik_per_m2 || material.verbruik exists then totaal = vloer_netto_m2 * (material.verbruik_per_m2 ?? material.verbruik); else requires_manual_input',
      pack_handling: 'if packaging count known then aantal = ceil(totaal / verpakkingseenheid) else aantal = ceil(totaal)',
...
function createGevelbekledingRuleSet(config: {
  ...
  return {
    ...
    folie_buiten: {
      ...
      formula: 'gevel_bruto_m2 = (gevel_lengte_mm * gevel_hoogte_mm) / 1000000; openingen_m2 = sum((opening_width_mm * opening_height_mm) / 1000000); gevel_netto_m2 = max(0, gevel_bruto_m2 - openingen_m2); dekking_m2 = material.lengte_m * material.breedte_m; aantal = ceil((gevel_netto_m2) / dekking_m2)',
    ...
    isolatie_gevel: {
      ...
      formula: 'gevel_bruto_m2 = (gevel_lengte_mm * gevel_hoogte_mm) / 1000000; openingen_m2 = sum((opening_width_mm * opening_height_mm) / 1000000); gevel_netto_m2 = max(0, gevel_bruto_m2 - openingen_m2); element_m2 = material.lengte_m * material.breedte_m; stuks = ceil((gevel_netto_m2) / element_m2)',
      pack_handling: 'if materiaalnaam matches /(pak\\s*(\\d+)st|\\((\\d+)st)/i then aantal = ceil(stuks / pack_size) else aantal = stuks',
      required_inputs: ['maatwerk_item.lengte', 'maatwerk_item.hoogte', 'material.lengte', 'material.breedte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    ...
    waterslag: {
      ...
      formula: "openings_waterslag_m1 = sum(opening_width_mm / 1000 for opening in openings_source where opening.type in ['window', 'frame-outer', 'frame-inner']); if openings_waterslag_m1 > 0 then aantal = ceil((openings_waterslag_m1) / material.lengte_m); else requires_manual_input",
      required_inputs: ['maatwerk_item.openings', 'material.lengte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
...
function createHellendDakRuleSet(): Record<string, Record<string, any>> {
  return {
    constructieplaat: {
      ...
      formula: 'dak_bruto_m2 = (dak_lengte_mm * dak_hoogte_mm) / 1000000; openingen_m2 = sum((opening_width_mm * opening_height_mm) / 1000000 for opening in openings_source); dak_netto_m2 = max(0, dak_bruto_m2 - openingen_m2); plaat_m2 = material.lengte_m * material.breedte_m; aantal = ceil((dak_netto_m2) / plaat_m2)',
      required_inputs: ['maatwerk_item.lengte', 'maatwerk_item.hoogte', 'material.lengte', 'material.breedte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    isolatie_dak: {
      ...
      formula: 'dak_bruto_m2 = (dak_lengte_mm * dak_hoogte_mm) / 1000000; openingen_m2 = sum((opening_width_mm * opening_height_mm) / 1000000 for opening in openings_source); dak_netto_m2 = max(0, dak_bruto_m2 - openingen_m2); element_m2 = material.lengte_m * material.breedte_m; stuks = ceil((dak_netto_m2) / element_m2)',
      pack_handling: 'if materiaalnaam matches /(pak\\s*(\\d+)st|\\((\\d+)st)/i then aantal = ceil(stuks / pack_size) else aantal = stuks',
      required_inputs: ['maatwerk_item.lengte', 'maatwerk_item.hoogte', 'material.lengte', 'material.breedte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    ...
    folie_buiten: {
      ...
      group: 'isolatie',
      logic: 'oppervlakte dakvlak minus openingen',
      formula: 'dak_bruto_m2 = (dak_lengte_mm * dak_hoogte_mm) / 1000000; openingen_m2 = sum((opening_width_mm * opening_height_mm) / 1000000 for opening in openings_source); dak_netto_m2 = max(0, dak_bruto_m2 - openingen_m2); dekking_m2 = material.lengte_m * material.breedte_m; aantal = ceil((dak_netto_m2) / dekking_m2)',
      required_inputs: ['maatwerk_item.lengte', 'maatwerk_item.hoogte', 'material.lengte', 'material.breedte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
...
function createEpdmDakRuleSet(): Record<string, Record<string, any>> {
  return {
    ...
    epdm_lijm: {
      ...
      formula: 'dak_bruto_m2 = (dak_lengte_mm * dak_hoogte_mm) / 1000000; openingen_m2 = sum((opening_width_mm * opening_height_mm) / 1000000 for opening in openings_source); dak_netto_m2 = max(0, dak_bruto_m2 - openingen_m2); perimeter_mm = (2 * dak_lengte_mm) + (2 * dak_hoogte_mm); opstand_m2 = (perimeter_mm * dakrand_breedte_mm) / 1000000; basis_m2 = dak_netto_m2 + opstand_m2; if material.verbruik_per_m2 || material.verbruik exists then totaal = basis_m2 * (material.verbruik_per_m2 ?? material.verbruik); else requires_manual_input',
      pack_handling: 'if packaging count known then aantal = ceil(totaal / verpakkingseenheid) else aantal = ceil(totaal)',
...
function createGolfplaatDakRuleSet(): Record<string, Record<string, any>> {
  return {
    gordingen: {
      ...
      group: 'hout',
      logic: 'horizontale gordingen op balkafstand (h.o.h.) over de lengte, met rijlengte in de breedte en optionele tussenmuur-segmentatie',
      formula: 'dak_breedte_mm = (maatwerk_item.breedte ?? maatwerk_item.hoogte); count = max(0, ceil(dak_lengte_mm / gordingafstand_mm) - 1) + (maatwerk_item.includeTopBottomGording == true ? 2 : 0); if count <= 0 then aantal = 0; else if maatwerk_item.tussenmuur_vanaf_links_maat exists || maatwerk_item.tussenmuur exists then seg1_mm = max(0, (maatwerk_item.tussenmuur_vanaf_links_maat ?? maatwerk_item.tussenmuur)); seg2_mm = max(0, dak_breedte_mm - seg1_mm); stuks_per_rij = ceil(seg1_mm / material.lengte_mm) + ceil(seg2_mm / material.lengte_mm); aantal = ceil(count * stuks_per_rij); else totaal_mm = count * dak_breedte_mm; aantal = ceil((totaal_mm) / material.lengte_mm)',
      required_inputs: ['maatwerk_item.lengte', 'maatwerk_item.breedte || maatwerk_item.hoogte', 'maatwerk_item.balkafstand', 'material.lengte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    tengels: {
      ...
      formula: 'dak_breedte_mm = (maatwerk_item.breedte ?? maatwerk_item.hoogte); count = ceil(dak_breedte_mm / tengelafstand_mm) + 1; totaal_mm = count * dak_lengte_mm; aantal = ceil((totaal_mm) / material.lengte_mm)',
      required_inputs: ['maatwerk_item.lengte', 'maatwerk_item.breedte || maatwerk_item.hoogte', 'maatwerk_item.latafstand', 'material.lengte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    isolatie_dak: {
      ...
      group: 'isolatie',
      logic: 'oppervlakte dakvlak minus openingen met pack-detectie',
      formula: 'dak_breedte_mm = (maatwerk_item.breedte ?? maatwerk_item.hoogte); dak_bruto_m2 = (dak_lengte_mm * dak_breedte_mm) / 1000000; openingen_m2 = sum((opening_width_mm * opening_height_mm) / 1000000 for opening in openings_source); dak_netto_m2 = max(0, dak_bruto_m2 - openingen_m2); element_m2 = material.lengte_m * material.breedte_m; stuks = ceil((dak_netto_m2) / element_m2)',
      pack_handling: 'if materiaalnaam matches /(pak\\s*(\\d+)st|\\((\\d+)st)/i then aantal = ceil(stuks / pack_size) else aantal = stuks',
      required_inputs: ['maatwerk_item.lengte', 'maatwerk_item.breedte || maatwerk_item.hoogte', 'material.lengte', 'material.breedte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    folie: {
      ...
      group: 'isolatie',
      logic: 'oppervlakte dakvlak minus openingen',
      formula: 'dak_breedte_mm = (maatwerk_item.breedte ?? maatwerk_item.hoogte); dak_bruto_m2 = (dak_lengte_mm * dak_breedte_mm) / 1000000; openingen_m2 = sum((opening_width_mm * opening_height_mm) / 1000000 for opening in openings_source); dak_netto_m2 = max(0, dak_bruto_m2 - openingen_m2); dekking_m2 = material.lengte_m * material.breedte_m; aantal = ceil((dak_netto_m2) / dekking_m2)',
      required_inputs: ['maatwerk_item.lengte', 'maatwerk_item.breedte || maatwerk_item.hoogte', 'material.lengte', 'material.breedte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    ...
    lichtplaten: {
      ...
      group: 'dak',
      logic: 'op maatwerk invoer, expliciete materiaalinvoer of op aantal dakraam-openingen',
      formula: "if maatwerk_item.lichtplaten_aantal exists then aantal = ceil(maatwerk_item.lichtplaten_aantal); else if material.aantal exists then aantal = ceil(material.aantal); else if count(openings where opening.type == 'dakraam') > 0 then aantal = count(openings where opening.type == 'dakraam'); else requires_manual_input",
      required_inputs: ['maatwerk_item.lichtplaten_aantal || material.aantal || maatwerk_item.openings'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    nokstukken: {
      ...
      group: 'dak',
      logic: 'lineair over de nok',
      formula: 'dak_breedte_mm = (maatwerk_item.breedte ?? maatwerk_item.hoogte); lineair_m1 = dak_breedte_mm / 1000; aantal = ceil((lineair_m1) / material.lengte_m)',
    ...
    hoekstukken: {
      ...
      formula: 'lineair_m1 = (2 * dak_lengte_mm) / 1000; aantal = ceil((lineair_m1) / material.lengte_m)',
      required_inputs: ['maatwerk_item.lengte', 'material.lengte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    golfplaatschroeven: {
      ...
      formula: 'dak_breedte_mm = (maatwerk_item.breedte ?? maatwerk_item.hoogte); dak_bruto_m2 = (dak_lengte_mm * dak_breedte_mm) / 1000000; openingen_m2 = sum((opening_width_mm * opening_height_mm) / 1000000 for opening in openings_source); dak_netto_m2 = max(0, dak_bruto_m2 - openingen_m2); if material.verbruik_per_m2 || material.verbruik exists then totaal = dak_netto_m2 * (material.verbruik_per_m2 ?? material.verbruik); else requires_manual_input',
      pack_handling: 'if packaging count known then aantal = ceil(totaal / verpakkingseenheid) else aantal = ceil(totaal)',
      required_inputs: ['maatwerk_item.lengte', 'maatwerk_item.breedte || maatwerk_item.hoogte', 'material.verbruik_per_m2 || material.verbruik'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    dakgoot: {
  ...
  const rules: Record<string, Record<string, any>> = {
    ...
    [config.panelSectionKey]: {
      ...
      formula: config.variant === 'beton'
        ? 'aantal = ceil(panel_count)'
        : "if type_schutting == 'schermen' then stuks = panel_count; else if material.lengte && material.breedte then plaat_m2 = material.lengte_m * material.breedte_m; stuks = ceil(fence_area_m2 / plaat_m2); else requires_manual_input; aantal = ceil(stuks)",
...
function createLichtkoepelRuleSet(): Record<string, Record<string, any>> {
  return {
    koepel: {
      ...
      formula: 'if material.aantal exists then aantal = ceil(material.aantal); else aantal = ceil(unit_count)',
      required_inputs: ['maatwerk_item.aantal'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    opstand: {
      ...
      formula: "if material.aantal exists then aantal = ceil(material.aantal); else if material.eenheid == 'stuk' then aantal = ceil(unit_count); else if material.lengte exists then lineair_m1 = total_perimeter_m1; aantal = ceil((lineair_m1) / material.lengte_m); else requires_manual_input",
    ...
    dakbedekking: {
      ...
      formula: "if material.dekking_m2 exists then aantal = ceil((total_opening_area_m2) / material.dekking_m2); else if material.lengte && material.breedte then dekkings_m2 = material.lengte_m * material.breedte_m; aantal = ceil((total_opening_area_m2) / dekkings_m2); else if material.aantal exists then aantal = ceil(material.aantal); else requires_manual_input",
      required_inputs: ['maatwerk_item.breedte', 'maatwerk_item.hoogte', 'maatwerk_item.aantal'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    betimmering: {
      ...
      formula: 'if material.lengte && material.breedte then plaat_m2 = material.lengte_m * material.breedte_m; aantal = ceil((total_opening_area_m2) / plaat_m2); else if material.lengte exists then lineair_m1 = total_perimeter_m1; aantal = ceil((lineair_m1) / material.lengte_m); else if material.aantal exists then aantal = ceil(material.aantal); else requires_manual_input',
      required_inputs: ['maatwerk_item.breedte', 'maatwerk_item.hoogte', 'maatwerk_item.aantal'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
    plinten: {
      ...
      formula: 'lineair_m1 = total_perimeter_m1; aantal = ceil((lineair_m1) / material.lengte_m)',
      required_inputs: ['maatwerk_item.breedte', 'maatwerk_item.hoogte', 'maatwerk_item.aantal', 'material.lengte'],
      missing_input_behavior: 'requires_manual_input',
      wastePercentage: 'user_input',
    },
...
const STATIC_RULES_BY_SLUG: Record<string, Record<string, Record<string, any>>> = {
  ...
  'hbs-buiten-wand': {
    ...
    gevelbekleding: {
      ...
      formula: '((total_height / effective_width) * total_length)',
  ...
  'plafond-houten-framework': {
    plinten_plafond: {
      ...
      formula: 'ceil((perimeter) / material.lengte)',
      sectionKey: 'plinten_plafond',
    },
    beplating: {
    ...
    rachels: {
      ...
      formula: 'ceil((count * length_per_rachel) / material.lengte)',
