...
export function createChecklistReminderHandler(kind: ChecklistKind) {
  ...
  return async function GET(request: Request): Promise<NextResponse> {
    ...
    try {
      ...
      const listsSnapshot = await firestore.collection(config.lists).where('userId', '==', uid).get();
      ...
      const items = itemsSnapshot.docs
        .map((doc) => {
          ...
          return {
            ...
            url: `${CALVORA_BASE_URL}${config.route}/${encodeURIComponent(listDoc.id)}`,
          };
        })
        .filter((item) => item.productName && item.quantity > 0)
        .filter((item) => !item.checked)
        .sort((left, right) => left.sortOrder - right.sortOrder)
        .map(({ id, productName, quantity, unit, notes, url }) => ({ id, productName, quantity, unit, notes, url }));
      ...
      return NextResponse.json({
        ...
        count: items.length,
        listId: listDoc.id,
        listUrl: `${CALVORA_BASE_URL}${config.route}/${encodeURIComponent(listDoc.id)}`,
        timezone: AMSTERDAM_TIME_ZONE,
        checkedAt: new Date().toISOString(),
        items,
      }, { headers: { 'Cache-Control': 'no-store' } });
    } catch (error) {
      console.error(`Openstaande ${config.plural} voor Telegram-herinnering ophalen mislukt:`, error);
      return NextResponse.json({ ok: false, error: `Openstaande ${config.plural} controleren mislukt.` }, { status: 500 });
    }
  };
}
