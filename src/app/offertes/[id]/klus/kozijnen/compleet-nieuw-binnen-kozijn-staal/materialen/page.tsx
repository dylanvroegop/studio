...
const sectieSleutels = ['kozijn', 'bevestiging', 'afwerking', 'extra'] as const;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Sla de huidige materiaalconfiguratie op voor later gebruik bij Stalen Binnenkozijnen.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
            <div className="space-y-2">
                <Label htmlFor="preset-name">Naam voorinstelling *</Label>
                <Input id="preset-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="bv. Standaard stalen kozijn" />
            </div>
            <div className="flex items-center space-x-2">
                <Checkbox id="default-preset" checked={isDefault} onCheckedChange={(checked) => setIsDefault(checked as boolean)} />
                <Label htmlFor="default-preset">Maak dit mijn standaard voor Stalen Binnenkozijnen</Label>
            </div>
        </div>
        <DialogFooter>
...
export default function CompleetNieuwBinnenKozijnStaalMaterialenPage() {
  ...
  const JOB_TYPE = "compleet-nieuw-binnen-kozijn-staal";
        ...
        <header className="sticky top-0 z-10 grid h-14 w-full grid-cols-3 items-center border-b bg-background/95 px-4 backdrop-blur-sm sm:px-6">
          <div className="flex items-center justify-start">
            <Button asChild variant="ghost" size="icon" className="h-8 w-8">
              <Link href={`/offertes/${quoteId}/klus/kozijnen/compleet-nieuw-binnen-kozijn-staal`}>
        ...
        <div className="flex-1 p-4 md:p-8">
          <div className="max-w-2xl mx-auto w-full">
              <div className="text-center mb-8">
                   <h1 className="font-semibold text-2xl md:text-3xl">Materialen – Compleet nieuw binnen kozijn – Staal</h1>
                  <p className="text-muted-foreground mt-2">
                      Kies de materialen die u voor dit kozijn gebruikt. U kunt deze keuzes als voorinstelling opslaan.
                  </p>
              </div>
              ...
              <div className="space-y-4">
                {renderSelectieRij('kozijn', 'Stalen kozijn')}
                {renderSelectieRij('bevestiging', 'Bevestiging')}
                {renderSelectieRij('afwerking', 'Afwerking (kit, etc)')}
              ...
              <div className="mt-8 flex justify-between items-center">
                  <Button variant="outline" asChild>
                      <Link href={`/offertes/${quoteId}/klus/kozijnen/compleet-nieuw-binnen-kozijn-staal`}>Terug</Link>
                  </Button>
                  <div>
