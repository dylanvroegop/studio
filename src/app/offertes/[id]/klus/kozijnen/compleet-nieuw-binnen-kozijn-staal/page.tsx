...
export default function CompleetNieuwBinnenKozijnStaalPage() {
  ...
  const handleSubmit = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (kozijnen.some(p => !p.breedte || !p.hoogte)) {
        toast({
            variant: "destructive",
            title: "Ontbrekende gegevens",
            description: "Vul a.u.b. de breedte en hoogte voor alle kozijnen in.",
        });
        return;
    }

    localStorage.setItem(`quote-${quoteId}-compleet-nieuw-binnen-kozijn-staal`, JSON.stringify(kozijnen));
    
    router.push(`/offertes/${quoteId}/klus/kozijnen/compleet-nieuw-binnen-kozijn-staal/materialen`);
  };
  
  const isNextDisabled = kozijnen.some(p => !p.breedte || !p.hoogte);
      ...
      <div className="flex-1 p-4 md:p-8">
        <div className="max-w-2xl mx-auto w-full">
            <div className="text-center mb-8">
                 <h2 className="font-semibold text-2xl">Compleet nieuw binnen kozijn – Staal</h2>
                       ...
                       <CardContent className="space-y-6">
                           <div className="grid grid-cols-2 gap-4">
                               <div className="space-y-2">
                                 ...
                                 <Input id={`breedte-${index}`} type="number" placeholder="Bijv. 930" required value={kozijn.breedte} onChange={(e) => handleKozijnChange(index, 'breedte', e.target.value)} />
                               </div>
                               <div className="space-y-2">
                                 <Label htmlFor={`hoogte-${index}`}>Hoogte (mm) *</Label>
                                 <Input id={`hoogte-${index}`} type="number" placeholder="Bijv. 2115" required value={kozijn.hoogte} onChange={(e) => handleKozijnChange(index, 'hoogte', e.target.value)} />
                               </div>
                           </div>
