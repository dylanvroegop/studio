...
const sectieSleutels = ['dakbeschot', 'isolatie', 'folie', 'panlatten', 'dakpannen', 'bevestiging', 'extra', 'klein_materiaal'] as const;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Sla de huidige materiaalconfiguratie op voor later gebruik bij Hellende Daken.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
            <div className="space-y-2">
                <Label htmlFor="preset-name">Naam voorinstelling *</Label>
                <Input id="preset-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="bv. Standaard zadeldak" />
            </div>
            <div className="flex items-center space-x-2">
                <Checkbox id="default-preset" checked={isDefault} onCheckedChange={(checked) => setIsDefault(checked as boolean)} />
                <Label htmlFor="default-preset">Maak dit mijn standaard voor Hellende Daken</Label>
            </div>
        </div>
        <DialogFooter>
...
export default function HellendDakVervangenMaterialenPage() {
  ...
  const JOB_TYPE = "hellend-dak-vervangen";
        ...
        <header className="sticky top-0 z-10 grid h-14 w-full grid-cols-3 items-center border-b bg-background/95 px-4 backdrop-blur-sm sm:px-6">
          <div className="flex items-center justify-start">
            <Button asChild variant="ghost" size="icon" className="h-8 w-8">
              <Link href={`/offertes/${quoteId}/klus/dakrenovatie/hellend-dak-vervangen`}>
        ...
        <div className="flex-1 p-4 md:p-8">
          <div className="max-w-2xl mx-auto w-full">
              <div className="text-center mb-8">
                   <h1 className="font-semibold text-2xl md:text-3xl">Materialen – Hellend Dak Vervangen</h1>
              ...
              <div className="space-y-4">
                {renderSelectieRij('dakbeschot', 'Dakbeschot')}
                {renderSelectieRij('isolatie', 'Isolatie')}
                {renderSelectieRij('folie', 'Folie')}
                {renderSelectieRij('panlatten', 'Panlatten')}
                {renderSelectieRij('dakpannen', 'Dakpannen')}
              ...
              <div className="mt-8 flex justify-between items-center">
                  <Button variant="outline" asChild>
                      <Link href={`/offertes/${quoteId}/klus/dakrenovatie/hellend-dak-vervangen`}>Terug</Link>
                  </Button>
                  <div>
