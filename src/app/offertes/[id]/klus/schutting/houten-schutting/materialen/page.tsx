...
const sectieSleutels = ['palen', 'schermen', 'bevestiging', 'extra'] as const;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Sla de huidige materiaalconfiguratie op voor later gebruik bij Schuttingen.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
            <div className="space-y-2">
                <Label htmlFor="preset-name">Naam voorinstelling *</Label>
                <Input id="preset-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="bv. Standaard Douglas schutting" />
            </div>
            <div className="flex items-center space-x-2">
                <Checkbox id="default-preset" checked={isDefault} onCheckedChange={(checked) => setIsDefault(checked as boolean)} />
                <Label htmlFor="default-preset">Maak dit mijn standaard voor Schuttingen</Label>
            </div>
        </div>
        <DialogFooter>
...
export default function HoutenSchuttingMaterialenPage() {
  ...
  const JOB_TYPE = "houten-schutting";
        ...
        <header className="sticky top-0 z-10 grid h-14 w-full grid-cols-3 items-center border-b bg-background/95 px-4 backdrop-blur-sm sm:px-6">
          <div className="flex items-center justify-start">
            <Button asChild variant="ghost" size="icon" className="h-8 w-8">
              <Link href={`/offertes/${quoteId}/klus/schutting/houten-schutting`}>
        ...
        <div className="flex-1 p-4 md:p-8">
          <div className="max-w-2xl mx-auto w-full">
              <div className="text-center mb-8">
                   <h1 className="font-semibold text-2xl md:text-3xl">Materialen – Houten Schutting</h1>
                  <p className="text-muted-foreground mt-2">
                      Kies de materialen die u voor deze schutting gebruikt. U kunt deze keuzes als voorinstelling opslaan.
                  </p>
              </div>
              ...
              <div className="space-y-4">
                {renderSelectieRij('palen', 'Palen')}
                {renderSelectieRij('schermen', 'Schuttingdelen / schermen')}
                {renderSelectieRij('bevestiging', 'Bevestiging (schroeven)')}
              ...
              <div className="mt-8 flex justify-between items-center">
                  <Button variant="outline" asChild>
                      <Link href={`/offertes/${quoteId}/klus/schutting/houten-schutting`}>Terug</Link>
                  </Button>
                  <div>
