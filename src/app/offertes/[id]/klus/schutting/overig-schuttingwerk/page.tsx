...
export default function OverigSchuttingwerkPage() {
  ...
  const handleSubmit = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (items.some(p => !p.lengte)) {
        toast({
            variant: "destructive",
            title: "Ontbrekende gegevens",
            description: "Vul a.u.b. de lengte voor alle items in.",
        });
        return;
    }
  ...
  return (
    <main className="flex flex-1 flex-col">
      <header className="grid h-14 w-full grid-cols-3 items-center border-b bg-background px-4 py-3 sm:px-6">
        ...
        <div className="text-center">
            <h1 className="font-semibold text-lg">Overig Schuttingwerk:</h1>
                   ...
                   <Card key={index}>
                       <CardHeader className="flex flex-row items-center justify-between">
                           <div>
                               ...
                               <CardDescription>
                                   Specificeer de afmetingen en details voor dit item.
                               </CardDescription>
                           </div>
                            {index > 0 && (
