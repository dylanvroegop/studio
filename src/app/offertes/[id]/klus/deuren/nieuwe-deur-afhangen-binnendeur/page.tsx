...
const defaultDeurState: Deur = {
  lengte: '',
  hoogte: '',
  opmerkingen: '',
};

export default function NieuweDeurAfhangenBinnendeurPage() {
  ...
  const [deuren, setDeuren] = useState<Deur[]>([defaultDeurState]);
  ...
  const handleAddDeur = () => {
    setDeuren([...deuren, { ...defaultDeurState }]);
  };
  
  const handleRemoveDeur = (index: number) => {
    const newDeuren = deuren.filter((_, i) => i !== index);
    setDeuren(newDeuren);
  };

  const handleDeurChange = <K extends keyof Deur>(index: number, field: K, value: Deur[K]) => {
    const newDeuren = [...deuren];
    newDeuren[index][field] = value;
    setDeuren(newDeuren);
  };
  ...
  const handleSubmit = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (deuren.some(p => !p.lengte || !p.hoogte)) {
        toast({
            variant: "destructive",
            title: "Ontbrekende gegevens",
            description: "Vul a.u.b. de lengte en hoogte voor alle deuren in.",
        });
        return;
    }

    localStorage.setItem(`quote-${quoteId}-binnendeuren`, JSON.stringify(deuren));
    
    router.push(`/offertes/${quoteId}/klus/deuren/nieuwe-deur-afhangen-binnendeur/materialen`);
  };
                ...
                {deuren.map((deur, index) => (
                   <Card key={index}>
                       <CardHeader className="flex flex-row items-center justify-between">
                           <div>
                               <CardTitle>Deur {index + 1}</CardTitle>
                               <CardDescription>
                                   Specificeer de afmetingen en details voor deze deur.
                               </CardDescription>
                           </div>
                            {index > 0 && (
                                <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveDeur(index)} className="h-8 w-8 text-muted-foreground hover:text-destructive flex-shrink-0 -mr-2">
                                    <Trash2 className="h-4 w-4" />
                                    <span className="sr-only">Verwijder deur</span>
                                </Button>
                            )}
                       </CardHeader>
                       <CardContent className="space-y-6">
                           ...
                           <div className="space-y-2 pt-2">
                              ...
                              <Textarea id={`opmerkingen-${index}`} placeholder="Bijzondere details, alleen indien nodig…" value={deur.opmerkingen} onChange={(e) => handleDeurChange(index, 'opmerkingen', e.target.value)} />
                            </div>
                       </CardContent>
                   </Card>
                ))}
              </div>
              <Button type="button" variant="outline" className="w-full mt-6" onClick={handleAddDeur}>
