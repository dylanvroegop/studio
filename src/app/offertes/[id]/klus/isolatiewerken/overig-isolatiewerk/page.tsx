...
const defaultVlakState: Vlak = {
  lengte: '',
  breedte: '',
  opmerkingen: '',
};

export default function OverigIsolatiewerkPage() {
  ...
  const [vlakken, setVlakken] = useState<Vlak[]>([defaultVlakState]);
  ...
  const handleAddVlak = () => {
    setVlakken([...vlakken, { ...defaultVlakState }]);
  };
  
  const handleRemoveVlak = (index: number) => {
    const newVlakken = vlakken.filter((_, i) => i !== index);
    setVlakken(newVlakken);
  };

  const handleVlakChange = <K extends keyof Vlak>(index: number, field: K, value: Vlak[K]) => {
    const newVlakken = [...vlakken];
    newVlakken[index][field] = value;
    setVlakken(newVlakken);
  };
  ...
  const handleSubmit = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (vlakken.some(p => !p.lengte || !p.breedte)) {
        toast({
            variant: "destructive",
            title: "Ontbrekende gegevens",
            description: "Vul a.u.b. de lengte en breedte voor alle vlakken in.",
        });
        return;
    }
      ...
      <div className="flex-1 p-4 md:p-8">
        <div className="max-w-2xl mx-auto w-full">
            <div className="text-center mb-8">
                 <h2 className="font-semibold text-2xl">Overig Isolatiewerk</h2>
                <p className="text-muted-foreground mt-2">
                    Vul hieronder de afmetingen in. Voor elk apart vlak, voeg een nieuw veld toe.
                </p>
            </div>
            <form>
              <div className="space-y-6">
                {vlakken.map((vlak, index) => (
                   <Card key={index}>
                       <CardHeader className="flex flex-row items-center justify-between">
                           <div>
                               <CardTitle>Vlak {index + 1}</CardTitle>
                               <CardDescription>
                                   Specificeer de afmetingen voor dit vlak.
                               </CardDescription>
                           </div>
                            {index > 0 && (
                                <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveVlak(index)} className="h-8 w-8 text-muted-foreground hover:text-destructive flex-shrink-0 -mr-2">
                                    <Trash2 className="h-4 w-4" />
                                    <span className="sr-only">Verwijder vlak</span>
                                </Button>
                            )}
                       </CardHeader>
                       <CardContent className="space-y-6">
                           ...
                           <div className="space-y-2 pt-2">
                              ...
                              <Textarea id={`opmerkingen-${index}`} placeholder="Bijzondere details, alleen indien nodig…" value={vlak.opmerkingen} onChange={(e) => handleVlakChange(index, 'opmerkingen', e.target.value)} />
                            </div>
                       </CardContent>
                   </Card>
                ))}
              </div>
              <Button type="button" variant="outline" className="w-full mt-6" onClick={handleAddVlak}>
