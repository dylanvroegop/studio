    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Sla de huidige materiaalconfiguratie op voor later gebruik bij Dak isoleren (binnenzijde).
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
            <div className="space-y-2">
                <Label htmlFor="preset-name">Naam voorinstelling *</Label>
                <Input id="preset-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="bv. Standaard PIR dak" />
            </div>
            <div className="flex items-center space-x-2">
                <Checkbox id="default-preset" checked={isDefault} onCheckedChange={(checked) => setIsDefault(checked as boolean)} />
                <Label htmlFor="default-preset">Maak dit mijn standaard voor Dak isoleren (binnenzijde)</Label>
            </div>
        </div>
        <DialogFooter>
...
export default function DakIsolerenBinnenzijdeMaterialenPage() {
  ...
  const JOB_TYPE = "dak-isoleren-binnenzijde";
