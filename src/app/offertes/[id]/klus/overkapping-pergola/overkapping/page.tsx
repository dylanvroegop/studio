...
export default function OverkappingPage() {
  ...
  const handleSubmit = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (items.some(p => !p.lengte || !p.breedte)) {
        toast({
            ...
            description: "Vul a.u.b. de lengte en breedte voor alle overkappingen in.",
        });
        return;
    }

    localStorage.setItem(`quote-${quoteId}-overkapping`, JSON.stringify(items));
    
    router.push(`/offertes/${quoteId}/klus/overkapping-pergola/overkapping/materialen`);
  };
      ...
      <header className="sticky top-0 z-10 grid h-14 w-full grid-cols-3 items-center border-b bg-background/95 px-4 backdrop-blur-sm sm:px-6">
        <div className="flex items-center justify-start">
          <Button asChild variant="ghost" size="icon" className="h-8 w-8">
            <Link href={`/offertes/${quoteId}/klus/overkapping-pergola`}>
              <ArrowLeft className="h-4 w-4" />
              <span className="sr-only">Terug</span>
            </Link>
          </Button>
        </div>
        <h1 className="text-center font-semibold text-lg">Overkapping: stap 4 van 6</h1>
      ...
      <div className="flex-1 p-4 md:p-8">
        <div className="max-w-2xl mx-auto w-full">
            <div className="text-center mb-8">
                 <h2 className="font-semibold text-2xl">Overkapping</h2>
                <p className="text-muted-foreground mt-2">
                    Vul hieronder de afmetingen in. Voor elke aparte overkapping, voeg een nieuw veld toe.
                </p>
            </div>
            <form>
                ...
                {items.map((item, index) => (
                   <Card key={index}>
                       <CardHeader className="flex flex-row items-center justify-between">
                           <div>
                               <CardTitle>Overkapping {index + 1}</CardTitle>
                               <CardDescription>
              ...
              <Button type="button" variant="outline" className="w-full mt-6" onClick={handleAddItem}>
                ...
                Overkapping toevoegen
              </Button>
              <div className="mt-6 flex justify-between items-center">
                  <Button variant="outline" asChild>
                      <Link href={`/offertes/${quoteId}/klus/overkapping-pergola`}>Terug</Link>
                  </Button>
