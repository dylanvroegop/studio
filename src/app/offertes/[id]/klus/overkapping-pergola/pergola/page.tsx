  ...
  const handleSubmit = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (items.some(p => !p.lengte || !p.breedte)) {
        toast({
            ...
            description: "Vul a.u.b. de lengte en breedte voor alle pergola's in.",
        });
        return;
    }

    localStorage.setItem(`quote-${quoteId}-pergola`, JSON.stringify(items));
    
    router.push(`/offertes/${quoteId}/klus/overkapping-pergola/pergola/materialen`);
  };
  ...
  return (
    <main className="flex flex-1 flex-col">
      <header className="sticky top-0 z-10 grid h-14 w-full grid-cols-3 items-center border-b bg-background/95 px-4 backdrop-blur-sm sm:px-6">
        ...
        <h1 className="text-center font-semibold text-lg">Pergola: stap 4 van 6</h1>
      ...
      <div className="flex-1 p-4 md:p-8">
        <div className="max-w-2xl mx-auto w-full">
            <div className="text-center mb-8">
                 <h2 className="font-semibold text-2xl">Pergola</h2>
                <p className="text-muted-foreground mt-2">
                    Vul hieronder de afmetingen in. Voor elke aparte pergola, voeg een nieuw veld toe.
                </p>
            </div>
            <form>
                ...
                {items.map((item, index) => (
                   <Card key={index}>
                       <CardHeader className="flex flex-row items-center justify-between">
                           <div>
                               <CardTitle>Pergola {index + 1}</CardTitle>
                               <CardDescription>
