...
export default function OmkastingenEnKovenPage() {
  ...
  const handleSubmit = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (items.some(p => !p.lengte || !p.hoogte || !p.diepte)) {
        toast({
            variant: "destructive",
            title: "Ontbrekende gegevens",
            description: "Vul a.u.b. de lengte, hoogte en diepte voor alle items in.",
        });
        return;
    }

    localStorage.setItem(`quote-${quoteId}-omkastingen-en-koven`, JSON.stringify(items));
    
    router.push(`/offertes/${quoteId}/klus/afwerkingen/omkastingen-en-koven/materialen`);
  };
  
  const isNextDisabled = items.some(p => !p.lengte || !p.hoogte || !p.diepte);
      ...
      <div className="flex-1 p-4 md:p-8">
        <div className="max-w-2xl mx-auto w-full">
            <div className="text-center mb-8">
                 <h2 className="font-semibold text-2xl">Omkastingen en koven</h2>
                ...
                {items.map((item, index) => (
                   <Card key={index}>
                       <CardHeader className="flex flex-row items-center justify-between">
                           <div>
                               <CardTitle>Omkasting / Koof {index + 1}</CardTitle>
                               <CardDescription>
                       ...
                       <CardContent className="space-y-6">
                           <div className="grid grid-cols-3 gap-4">
                               <div className="space-y-2">
                                 ...
                                 <Input id={`lengte-${index}`} type="number" placeholder="Bijv. 1200" required value={item.lengte} onChange={(e) => handleItemChange(index, 'lengte', e.target.value)} />
                               </div>
                               <div className="space-y-2">
                                 <Label htmlFor={`hoogte-${index}`}>Hoogte (mm) *</Label>
                                 <Input id={`hoogte-${index}`} type="number" placeholder="Bijv. 900" required value={item.hoogte} onChange={(e) => handleItemChange(index, 'hoogte', e.target.value)} />
                               </div>
                               <div className="space-y-2">
                                 <Label htmlFor={`diepte-${index}`}>Diepte (mm) *</Label>
                                 <Input id={`diepte-${index}`} type="number" placeholder="Bijv. 200" required value={item.diepte} onChange={(e) => handleItemChange(index, 'diepte', e.target.value)} />
                               </div>
                           </div>
