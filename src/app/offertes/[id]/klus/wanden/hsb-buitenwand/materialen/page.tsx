...
const sectieSleutels = ['balktype', 'isolatie', 'folie_buitenzijde', 'binnenbekleding', 'gips_fermacell', 'buitenbekleding', 'extra', 'klein_materiaal'] as const;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Sla de huidige materiaalconfiguratie op voor later gebruik bij HSB Buitenwanden.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
            <div className="space-y-2">
                <Label htmlFor="preset-name">Naam voorinstelling *</Label>
                <Input id="preset-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="bv. Standaard gevelwand" />
            </div>
            <div className="flex items-center space-x-2">
                <Checkbox id="default-preset" checked={isDefault} onCheckedChange={(checked) => setIsDefault(checked as boolean)} />
                <Label htmlFor="default-preset">Maak dit mijn standaard voor HSB Buitenwanden</Label>
            </div>
        </div>
        <DialogFooter>
...
export default function HsbBuitenwandMaterialenPage() {
  ...
  useEffect(() => {
    ...
    const fetchPresets = async () => {
      ...
      try {
        const q = query(collection(firestore, 'presets'), where('userId', '==', user.uid), where('jobType', '==', JOB_TYPE));
        ...
        <div className="flex-1 p-4 md:p-8">
          <div className="max-w-2xl mx-auto w-full">
              <div className="text-center mb-8">
                   <h1 className="font-semibold text-2xl md:text-3xl">Materialen – HSB Buitenwand (gevel)</h1>
              ...
              <div className="mt-8 flex justify-between items-center">
                  <Button variant="outline" asChild>
                      <Link href={`/offertes/${quoteId}/klus/wanden/hsb-buitenwand`}>Terug</Link>
                  </Button>
                  <div>
                    <Button className="bg-primary text-primary-foreground hover:bg-primary/90">Volgende</Button>
                  </div>
              </div>
          </div>
        </div>
