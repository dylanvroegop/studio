...
const sectieSleutels = ['rachelwerk', 'isolatie', 'folie', 'gips_fermacell', 'naden_vullen', 'extra'] as const;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Sla de huidige materiaalconfiguratie op voor later gebruik bij Gipsplafonds met houten framewerk.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
            <div className="space-y-2">
                <Label htmlFor="preset-name">Naam voorinstelling *</Label>
                <Input id="preset-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="bv. Standaard plafond" />
            </div>
            <div className="flex items-center space-x-2">
                <Checkbox id="default-preset" checked={isDefault} onCheckedChange={(checked) => setIsDefault(checked as boolean)} />
                <Label htmlFor="default-preset">Maak dit mijn standaard voor Gipsplafonds met houten framewerk</Label>
            </div>
        </div>
        <DialogFooter>
...
export default function GipsplafondHoutenFramewerkMaterialenPage() {
  ...
  const JOB_TYPE = "gipsplafond-houten-framewerk";
