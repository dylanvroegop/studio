...
export function ChecklistDetail({ kind }: { kind: ChecklistKind }) {
  const config = CHECKLIST_CONFIG[kind];
  ...
  useEffect(() => {
    ...
    const unsubscribeList = onSnapshot(
      doc(firestore, config.lists, id),
      (snapshot) => {
        if (!snapshot.exists()) {
          setError(`${config.title} niet gevonden.`);
          setList(null);
          setLoading(false);
          return;
        }
        ...
        if (data.userId !== user.uid) {
          setError(`Geen toegang tot deze lijst.`);
          setList(null);
          setLoading(false);
          return;
        }
      ...
      (err: unknown) => {
        setError(getErrorMessage(err, `${config.title} kon niet worden geladen.`));
        setLoading(false);
      }
    );
    const unsubscribeItems = onSnapshot(
      query(collection(firestore, config.items), where('material_list_id', '==', id)),
      (snapshot) => {
      ...
      (err: unknown) => setError(getErrorMessage(err, `${config.plural} konden niet worden geladen.`))
    );
    return () => {
      unsubscribeList();
      unsubscribeItems();
    };
  }, [config, firestore, id, user]);
  ...
  async function touchList(): Promise<void> {
    ...
    await updateDoc(doc(firestore, config.lists, id), {
  ...
  async function addItem(inputOverride?: string): Promise<boolean> {
    ...
    try {
      ...
      if (!duplicate) {
        await addDoc(collection(firestore, config.items), {
          material_list_id: id,
          product_name: name,
          quantity: 1,
          unit: 'st',
      ...
      setError(getErrorMessage(err, `${config.itemLabel} kon niet worden toegevoegd.`));
      return false;
    } finally {
      setSaving(false);
    }
  ...
  async function toggleItem(item: MaterialListItem): Promise<void> {
    if (!firestore) return;
    try {
      await updateDoc(doc(firestore, config.items, item.id), { checked: !item.checked });
    } catch (err: unknown) {
      setError(getErrorMessage(err, `${config.itemLabel} kon niet worden verplaatst.`));
    }
  }
  ...
  async function deleteItem(item: MaterialListItem): Promise<void> {
    if (!firestore) return;
    try {
      await deleteDoc(doc(firestore, config.items, item.id));
      await touchList();
    } catch (err: unknown) {
      setError(getErrorMessage(err, `${config.itemLabel} kon niet worden verwijderd.`));
    }
  }
  ...
  if (!list) {
    return (
      <div className="app-shell min-h-screen bg-background">
        <AppNavigation />
        <DashboardHeader user={user} title={config.title} hideAccountOnMobile />
        <main className="mx-auto max-w-3xl p-4"><Card><CardContent className="py-10 text-center text-sm text-muted-foreground">{error || `${config.title} niet gevonden.`}</CardContent></Card></main>
      </div>
    );
  }
    ...
    <div className="app-shell min-h-screen bg-background">
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-4 pb-24 md:px-6 md:py-5 md:pb-14">
        <div className="space-y-4 md:space-y-5">
          <ChecklistTabs kind={kind} />
          <div className="flex items-center justify-between gap-3">
            <div>
              <h1 className="text-xl font-semibold">{list.title || config.defaultTitle}</h1>
            ...
            <CardContent className="p-3 sm:p-4">
              <div className="flex gap-2">
                <Input
                  ...
                  aria-label={`${config.itemLabel} toevoegen`}
                />
                <Button className="h-12 w-12 shrink-0 px-0" onClick={() => void addItem()} disabled={saving || !quickAdd.trim()} aria-label={`${config.itemLabel} toevoegen`}>
            ...
            {visibleItems.length === 0 ? (
              <Card><CardContent className="flex flex-col items-center gap-3 py-12 text-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{tab === 'active' ? `Voeg een ${config.noun} toe met de + knop.` : `Afgevinkte ${config.plural} verschijnen hier.`}</p>
                </div>
              </CardContent></Card>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{config.itemLabel} toevoegen</DialogTitle>
            <DialogDescription>Voer de naam in van het {config.noun} dat je wilt halen.</DialogDescription>
          </DialogHeader>
          <form
            onSubmit={(event) => {
              event.preventDefault();
