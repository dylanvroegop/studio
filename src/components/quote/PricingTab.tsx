...
export function PricingTab({ quoteId, quote, quoteTitle, notes, vatRate = 21 }: PricingTabProps) {
    const { user } = useUser();
    const firestore = useFirestore();
    const { toast } = useToast();
    const [lines, setLines] = useState<QuotePriceLine[]>([]);
    ...
    useEffect(() => {
        ...
        const loadRules = async () => {
            try {
                ...
                if (!cancelled) setRules(sanitizeQuotePriceRules(snapshot.data()?.prijsregels));
            } catch (error) {
    ...
    const totalExclBtw = useMemo(
        () => lines.reduce((total, line) => total + calculateQuotePriceLineTotal(line), 0),
        [lines],
    );
    ...
    const applySuggestions = () => {
        ...
        setLines(suggestions.map((line) => ({ ...line, id: createQuotePricingId('regel') })));
        setSuggestions(null);
    ...
    const savePricing = async () => {
        ...
        const cleanedLines = sanitizeQuotePriceLines(lines).filter((line) => line.title.trim());
        setIsSaving(true);
        try {
            ...
            toast({ title: 'Prijsraming opgeslagen', description: 'De regels zijn opgeslagen bij deze offerte.' });
        } catch (error) {
            toast({
                variant: 'destructive',
    ...
    const saveRules = async () => {
        ...
        const candidateLines = lines.filter((line) => line.title.trim() && line.unitPriceExclBtw > 0);
        ...
        <div className="space-y-5 pb-32">
            <div className="rounded-xl border border-border bg-card p-4">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
                                Snelle verkoopprijs op basis van m², m¹, stuks, uren of een vast bedrag. De lange materiaalcalculatie blijft apart voor exacte aantallen.
                            </p>
                        </div>
                    </div>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-border/60 pt-3 text-xs text-muted-foreground">
                    ...
                    <span>Bron: goedgekeurde regels + oude offertes als context</span>
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <div className="flex items-center gap-2 font-medium text-amber-100">
                                ...
                                AI-voorstel controleren
                            </div>
                            <p className="mt-1 text-sm text-amber-100/75">
                                {suggestionMessage || 'Controleer elke regel voordat je deze toepast.'}
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-card">
                <div className="flex flex-col gap-2 border-b border-border px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h3 className="font-medium text-foreground">Prijsregels voor deze offerte</h3>
                        <p className="text-xs text-muted-foreground">Pas de AI-regels aan. De berekening is altijd hoeveelheid × prijs.</p>
                    </div>
                ...
                {lines.length === 0 ? (
                    <div className="px-4 py-10 text-center text-sm text-muted-foreground">
                        Nog geen prijsregels. Gebruik een AI-voorstel of voeg zelf een regel toe.
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-[860px] text-sm">
                            <thead className="border-b border-border bg-muted/20 text-left text-xs uppercase tracking-wide text-muted-foreground">
                                <tr>
                                    <th className="px-3 py-3">Onderdeel</th>
                                ...
                                {lines.map((line) => (
                                    <tr key={line.id} className="border-b border-border/50 last:border-0">
                                        ...
                                        <td className="px-3 py-2 text-right align-top">
                                            <Button type="button" variant="ghost" size="icon" onClick={() => removeLine(line.id)} aria-label="Verwijder prijsregel" title="Verwijder prijsregel">
                            ...
                            <tfoot className="border-t border-border bg-muted/20">
                                <tr>
                                    <td colSpan={4} className="px-3 py-3 text-right font-medium">Totaal prijsraming excl. btw</td>
                                    <td className="px-3 py-3 text-right font-mono font-semibold text-emerald-300">{formatMoney(totalExclBtw)}</td>
                                    <td colSpan={2} />
                                </tr>
                                <tr>
                                    <td colSpan={4} className="px-3 pb-3 text-right text-xs text-muted-foreground">Totaal incl. {vatRate}% btw</td>
                                    <td className="px-3 pb-3 text-right font-mono text-sm text-foreground">{formatMoney(totalInclBtw)}</td>
                                    <td colSpan={2} />
                                </tr>
                            </tfoot>
                        </table>
            ...
            <div className="rounded-xl border border-border bg-card p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Sla gecontroleerde regels op voor volgende offertes. Een bestaande titel + eenheid wordt bijgewerkt.</p>
                    </div>
                ...
                {rules.length > 0 ? (
                    <div className="mt-4 divide-y divide-border/60 rounded-lg border border-border/60">
                        {rules.map((rule) => {
                            const matchingLine = lines.some((line) => (
                                line.unit === rule.unit
            ...
            <div className="flex items-start gap-2 rounded-lg border border-blue-500/20 bg-blue-500/5 px-3 py-2 text-xs text-blue-100/75">
                ...
                <span>De AI doet alleen een eerste voorstel. Controleer vooral de prijs, de oppervlakteformule en wat wel of niet in de regel zit voordat je deze als vaste prijsregel bewaart.</span>
            </div>
        </div>
    );
}
