...
export interface JobCategoryPageProps {
  embedded?: boolean;
  categorySlug?: string;
  onBack?: () => void;
}
...
export function JobCategoryPage({
  ...
  const routeCategorySlug = params.category as string;
  const categorySlug = categorySlugProp || routeCategorySlug;
  ...
  return (
    <main className={cn(
      ...
      {/* STICKY SEARCH */}
      <div className={cn(
        'mobile-calm-pane bg-background sticky top-0 z-40 border-b shadow-sm max-w-5xl mx-auto w-full px-4',
        embedded ? 'pt-3 pb-3' : 'pt-4 pb-3',
      )}>
