...
import JobCategoryTab from '@/components/quote/JobCategoryTab';
...
export function NewJobPage({ embedded = false, onBack }: NewJobPageProps = {}) {
  ...
  }, [embedded, quoteId, firestore, router, startTransition, quote?.userId, toast, user?.uid]);

  if (!isMounted) return null;

  return (
    <main className={cn(
      'mobile-calm relative min-h-screen bg-background flex flex-col',
      embedded && 'h-[78vh] min-h-0',
    )}>
      {/* HEADER */}
      {!embedded && (
        <WizardHeader
      ...
      {embedded && selectedCategorySlug ? (
        <JobCategoryTab
          embedded
          categorySlug={selectedCategorySlug}
          onBack={() => setSelectedCategorySlug(null)}
        />
      ) : (
        <>
          {/* STICKY SEARCH */}
          <div className={cn(
            'mobile-calm-pane sticky top-0 z-40 bg-background/80 backdrop-blur-md px-4 border-b border-border/40',
            embedded ? 'pt-3 pb-3' : 'pt-4 pb-4',
          )}>
          ...
          {/* CONTENT */}
          <div className={cn(
            'flex-1 px-4 py-4 max-w-5xl mx-auto w-full space-y-6',
            embedded ? 'overflow-y-auto pb-6' : 'pb-40',
          )}>
        ...
        <section>
          ...
          {embedded ? (
            onBack ? (
              <div className="mobile-calm-pane sticky bottom-0 z-50 border-t border-border bg-background/95 backdrop-blur-sm">
