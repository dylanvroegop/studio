...
import NewJobTab from '@/components/quote/NewJobTab';
...
export function OverzichtPage({ embedded = false, onCalculated }: OverzichtPageProps = {}) {
  ...
  const [newJobPickerOpen, setNewJobPickerOpen] = useState(false);
  ...
  return (
    <main className={cn('mobile-calm flex min-h-screen flex-col', embedded && 'min-h-0')}>
      {embedded && (
        <Dialog open={newJobPickerOpen} onOpenChange={setNewJobPickerOpen}>
          <DialogContent className="w-[96vw] max-w-6xl overflow-hidden p-0">
            <DialogHeader className="sr-only">
              <DialogTitle>Een klus toevoegen</DialogTitle>
              <DialogDescription>Kies een klus om aan deze offerte toe te voegen.</DialogDescription>
            </DialogHeader>
            <NewJobTab embedded onBack={() => setNewJobPickerOpen(false)} />
          </DialogContent>
        </Dialog>
      )}
      ...
      <div className={cn(
        'flex-1 pb-[14rem] sm:pb-[9rem]',
        embedded ? 'px-0 py-0 pb-32 sm:pb-32' : 'px-4 py-6 md:py-10',
      )}>
