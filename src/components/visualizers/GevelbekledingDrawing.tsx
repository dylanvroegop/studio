...
export interface GevelbekledingDrawingProps {
...
export function GevelbekledingDrawing({
    lengte,
    hoogte,
    shape = 'rectangle',
    hoogteLinks,
    ...
    latten_orientation = 'horizontal',
    doubleEndBattens = false,
    openings = [],
    className,
    onOpeningsChange,
    fitContainer,
    ...
    return (
        ...
        >
            {(ctx) => {
                ...
                if (tengelafstandNum > 0) {
                    if (tengelOrientation === 'horizontal') {
                        ...
                        if (doubleEndTengels) {
                            ...
                            const yBottom = getY(extraBottom);
                            const halfWidthPx = (LAT_WIDTH * pxPerMm) / 2;
                ...
                // --- COMBINE LATTEN WITH BEAMS ---
                // We want latten to be visible, maybe above beams or below openings?
                // Usually below openings.
                ...
                const outlinePoints = outlinePointsMm
                    .map((p) => `${WALL_X + p.x * pxPerMm},${getY(p.y)}`)
                    .join(' ');
...
export default GevelbekledingDrawing;
